...
def convertToSVMinput(inputsourceVectors, inputsourceLabels,svmfile):
    ...
    for i in range(len(vectors)):
        ...
        for j in range(vectorDimension):
            svmfile.write(str(j))
            svmfile.write(":")
            svmfile.write(str(float(temp[j])))
            svmfile.write(" ")
        svmfile.write('\n')
    svmfile.flush()
    svmfile.close()
...
if __name__ == '__main__':
    ...
    all_senvector1 = []; all_senvector2 = []; all_senvector3 = [];all_senvector4 = [];all_senvector5 = [];all_senvector6 = [];
    ...
    for line in openfile:
        ...
        all_senvector2.append(linelist[4].lstrip().rstrip().split()) # exp ( log p(sentence | previous, following, w_n))
        
        all_senvector3.append(linelist[5].lstrip().rstrip().split()) # \sigma_N p(w_n|T) \sigma_Wn p(T | W_n)   for tfidf select
        
        all_senvector4.append(linelist[6].lstrip().rstrip().split()) # p(sentence | previous, following, w_n)  \sigma_N p(w_n|T) \sigma_Wn p(T | W_n)  for tfidf select
        
        all_senvector5.append(linelist[7].lstrip().rstrip().split()) # \sigma_N p(w_n|T) \sigma_Wn p(T | W_n)  for all
        
        all_senvector6.append(linelist[8].lstrip().rstrip().split()) # p(sentence | previous, following, w_n)  \sigma_N p(w_n|T) \sigma_Wn p(T | W_n)  for all
